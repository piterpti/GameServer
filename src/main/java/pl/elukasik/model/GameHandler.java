...
public class GameHandler implements Runnable {
	...
	private boolean sendToPlayer(final int playerId, Message msg) {
		
		PlayerConnector pConn = playerId == 1 ? connP1 : connP2;
		return pConn.sendMessage(msg);
		
	}
	
	private Message getPlayerResponse(int player) {
		
		Message msgFromPlayer = null;
		
		PlayerConnector pConn = player == 1 ? connP1 : connP2;
		...
		while (msgFromPlayer == null) {
			...
			msgFromPlayer = pConn.getMessage();
			try {
				Thread.sleep(10);
